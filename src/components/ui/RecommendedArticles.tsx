...
const RecommendedArticles: React.FC<RecommendedArticlesProp> = (props: RecommendedArticlesProp) => {
    ...
    return (<div id='recommended-articles' className={`flex flex-col w-full h-fit bg-[#EBEFFA] ${(className)? className:''}`} style={{...style}}>
        <div className='flex flex-row justify-between w-full h-fit min-h-[250px] sm:h-[88px]'>
        ...
        <div className="container flex flex-col items-center md-1:flex-col md-1:items-center md-2:flex-row lg-1:flex-col lg-1:items-center lg-2:flex-row flex-wrap justify-center w-full h-fit p-[28px_12px_28px] md:px-[36px] gap-x-[16px] gap-y-[8px] sm:gap-y-[40px]">
        </div>
    </div>);
}
